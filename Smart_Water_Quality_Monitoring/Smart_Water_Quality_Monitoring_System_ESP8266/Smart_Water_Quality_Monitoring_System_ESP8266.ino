...
const char* ssid = "project";//Replace with your network SSID
...
void setup()
{
  Serial.begin(9600);
  lcd.begin();
  lcd.print("IoT Base Hydro-");
  lcd.setCursor(0,1);
  lcd.print("ponics System");
...
while (WiFi.status() != WL_CONNECTED)
  ...
  server.on("/", []()
  {
    page = "<html><head><title>tech vision</title></head><style type=\"text/css\">";
    page += "table{border-collapse: collapse;}th {background-color:  skyblue ;color: red;}table,td {border: 4px solid black;font-size: x-large;";
    page += "text-align:center;border-style: groove;border-color: rgb(255,0,0);}</style><body><center>";
    page += "<h1>IoT Base Hydroponics System</h1><br><br><table style=\"width: 1200px;height: 450px;\"><tr>";
